...
public func settingsController(context: AccountContext, accountManager: AccountManager, enableDebugActions: Bool) -> SettingsController & ViewController {
    ...
    let arguments = SettingsItemArguments(sharedContext: context.sharedContext, avatarAndNameInfoContext: avatarAndNameInfoContext, avatarTapAction: {
        ...
        let _ = (contextValue.get()
            |> deliverOnMainQueue
            |> take(1)).start(next: { context in
                let resolveSignal = resolvePeerByName(account: context.account, name: "@cloudveilbot", ageLimit: 10)
                ...
                disposable.set((resolveSignal
                    ...
                    |> deliverOnMainQueue).start(next: { peer in
                        if let peer = peer {
                            pushControllerImpl?(context.sharedContext.makeChatController(context: context, chatLocation: .peer(peer.id), subject: nil, botStart: nil, mode: .standard(previewing: false)))
                        }
                    }))
            })/*
        let _ = (contextValue.get()
